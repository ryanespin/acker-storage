...
const regionDataSorted = Object.keys(regionDataGrouped)
  .sort(
    (a, b) =>
      (regionDataGrouped[b] ? regionDataGrouped[b].length : 0) -
      (regionDataGrouped[a] ? regionDataGrouped[a].length : 0)
  )
  .reduce(
    (acc, key) =>
      (
        // eslint-disable-next-line no-sequences
        (acc[key] = regionDataGrouped[key]), acc
      ),
    {} as keyof typeof regionDataGrouped
  );
      ...
      <Flex gap="sm">
        ...
        <Box flex={1}>
          {chartData.map((e, i) => {
            ...
            const highSum = filteredArr[0].reduce(
              (n, { estimate }) => n + (estimate?.high || 0),
              0
            );
            return (
              <Box key={i} mb="xs">
                <Flex className={classes['legend-container']}>
                  ...
                  <Box>
                    ...
                    <Text className={classes['legend-count']}>
                      {e.value} item{e.value !== 1 ? 's' : ''}
                    </Text>
                  </Box>
                  <Text className={classes['legend-value']}>
                    {makeCurrencyFriendly(lowSum, highSum)}
                  </Text>
                </Flex>
              </Box>
            );
